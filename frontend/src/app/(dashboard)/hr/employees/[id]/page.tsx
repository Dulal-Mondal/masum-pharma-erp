...
// ─── Daily Time Table ─────────────────────────────────────────────────────────
// Groups sessions by date, shows daily totals and overtime correctly

function DailyTimeTable({ entries, overtimeRate, onDelete }: {
    entries: TimeEntry[];
    overtimeRate: number;
    ...
    const totalWorkHours = entries.reduce((s, e) => s + Number(e.workHours ?? 0), 0);
    const totalOvertimeHours = entries.reduce((s, e) => s + Number(e.overtimeHours ?? 0), 0);
    ...
    return (
        <div className="space-y-3">
            {dates.map((date) => {
                ...
                const dayTotal = sessions.reduce((s, e) => s + Number(e.workHours ?? 0), 0);
                const dayOT = sessions.reduce((s, e) => s + Number(e.overtimeHours ?? 0), 0);
                const dayOTPay = dayOT * overtimeRate;
                    ...
                    <div key={date} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        ...
                        <div className="flex items-center justify-between px-5 py-2.5 bg-gray-50 border-b border-gray-100">
                            ...
                            <div className="flex items-center gap-4 text-xs">
                                <span className="text-blue-700 font-medium">Total: {dayTotal.toFixed(1)} hrs</span>
                                {dayOT > 0 && (
                                    <span className="bg-purple-100 text-purple-700 font-semibold px-2 py-0.5 rounded-full">
                                        OT: {dayOT.toFixed(1)} hrs = {formatCurrency(dayOTPay)}
                                    </span>
                                )}
                            </div>
                        </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="border-b border-gray-50">
                                <tr>
                                    <th className="text-left px-5 py-2 text-xs font-medium text-gray-400 uppercase">Session</th>
                                    <th className="text-center px-4 py-2 text-xs font-medium text-green-600 uppercase">Check In</th>
                                    <th className="text-center px-4 py-2 text-xs font-medium text-red-500 uppercase">Check Out</th>
                                    <th className="text-center px-4 py-2 text-xs font-medium text-blue-500 uppercase">Hours</th>
                                    <th className="text-center px-4 py-2 text-xs font-medium text-purple-500 uppercase">OT Hours</th>
                                    <th className="text-left px-4 py-2 text-xs font-medium text-gray-400 uppercase">Note</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {sessions.map((e, idx) => (
                                    <tr key={e.id} className="hover:bg-gray-50">
                                        <td className="px-5 py-2.5 text-gray-500 text-xs">Session {idx + 1}</td>
                                        ...
                                        <td className="px-4 py-2.5 text-center">
                                            {e.checkOut ? (
                                                <span className="font-medium text-red-600">
                                                    {e.checkOutDate && e.checkOutDate.split('T')[0] !== e.entryDate.split('T')[0]
                                                        ? <span className="text-xs text-gray-400 mr-1">{formatDate(e.checkOutDate)}</span>
                                                        : null}
                                                    {e.checkOut}
                                                </span>
                                            ) : <span className="text-gray-300">—</span>}
                                        ...
                                        <td className="px-4 py-2.5">
                                            <button
                                                onClick={() => { if (confirm('Delete this session?')) onDelete(e.id); }}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between">
                <span className="text-sm font-semibold text-blue-800">Month Total</span>
                <div className="flex items-center gap-6">
                    <div className="text-center">
                        <p className="text-xs text-blue-500">Work Hours</p>
                        <p className="text-lg font-bold text-blue-800">{totalWorkHours.toFixed(1)} hrs</p>
                    </div>
                    <div className="text-center">
                        <p className="text-xs text-purple-500">Overtime Hours</p>
                        <p className="text-lg font-bold text-purple-700">{totalOvertimeHours.toFixed(1)} hrs</p>
                    </div>
                    <div className="text-center">
                        <p className="text-xs text-green-500">Overtime Pay</p>
                        <p className="text-lg font-bold text-green-700">{formatCurrency(totalOvertimePay)}</p>
                    </div>
                </div>
            </div>
        </div>
