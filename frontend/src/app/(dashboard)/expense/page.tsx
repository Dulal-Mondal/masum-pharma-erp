...
import { expenseApi } from '@/lib/api';
import { formatCurrency, formatDate, today } from '@/lib/utils';
import { Plus, Trash2, Pencil, RefreshCcw, Loader2, FileSpreadsheet, FileDown } from 'lucide-react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import type { Expense, ExpenseCategory } from '@/types';
...
export default function ExpensePage() {
    ...
    const periodText = `${formatDate(dateRange.startDate)} — ${formatDate(dateRange.endDate)}`;
    ...
    const exportXLSX = async () => {
        ...
        try {
            ...
            const wsData = [
                ...
                [`Total (${expenses.length})`, '', '', formatCurrency(total)],
            ];
            const ws = XLSX.utils.aoa_to_sheet(wsData);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, 'Expense Report');
            XLSX.writeFile(wb, `${SHOP_NAME} - Expense Report - ${periodText}.xlsx`);
        } catch {
            alert('Run: npm install xlsx');
        } finally {
    ...
    const exportPDF = () => {
      ...
      <title>${SHOP_NAME} - Expense Report</title>
      <style>
      ...
      <table>
        ...
        <tbody>${rows}</tbody>
        <tfoot><tr>
          <td colspan="3" style="text-align:right">Total (${expenses.length} entries)</td>
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex items-center gap-2 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white text-sm font-medium px-3 py-2 rounded-lg transition-colors"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-medium px-3 py-2 rounded-lg transition-colors"
                    >
                        <FileDown size={13} />
                        PDF
                    </button>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
                <div className="flex flex-wrap gap-3 items-end">
                    ...
                    <button onClick={() => refetch()} className="flex items-center gap-1.5 border border-gray-300 text-gray-600 text-sm px-3 py-2 rounded-lg hover:bg-gray-50">
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                {isLoading ? (
                    <div className="flex items-center justify-center py-16"><RefreshCcw size={20} className="animate-spin text-gray-300" /></div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {expenses.length === 0 ? (
                                    <tr><td colSpan={5} className="text-center py-12 text-gray-400 text-sm">No expenses found</td></tr>
                                ) : (
                                    expenses.map((e) => (
                                        <tr key={e.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-5 py-3 font-medium text-gray-900">{e.category.categoryName}</td>
                                            <td className="px-5 py-3 text-gray-500">{e.note || '—'}</td>
                                            <td className="px-5 py-3 text-right font-semibold text-gray-900">{formatCurrency(Number(e.amount))}</td>
                                            <td className="px-5 py-3">
                                                <div className="flex items-center gap-2 justify-end">
                                                    <button onClick={() => { setEditItem(e); setShowForm(true); }} className="text-blue-400 hover:text-blue-600 p-1"><Pencil size={14} /></button>
                                                    <button onClick={() => { if (confirm('Delete this expense?')) deleteMutation.mutate(e.id); }} className="text-red-400 hover:text-red-600 p-1"><Trash2 size={14} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                            ...
                            {expenses.length > 0 && (
                                <tfoot>
                                    <tr className="bg-gray-50 border-t-2 border-gray-200">
                                        ...
                                        <td className="px-5 py-3 text-right text-base font-bold text-gray-900">{formatCurrency(total)}</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            )}
...
function ExpenseModal({ categories, editData, onClose, onSaved }: {
    categories: ExpenseCategory[];
    editData: Expense | null;
    onClose: () => void;
    onSaved: () => void;
    ...
    const onSubmit = async (data: FormValues) => {
        try {
            setApiError('');
            if (isEdit) await expenseApi.update(editData!.id, data);
    ...
    return (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
                <div className="px-6 py-4 border-b border-gray-100">
                    <h2 className="text-base font-semibold text-gray-900">{isEdit ? 'Edit Expense' : 'Add Expense'}</h2>
                </div>
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                    <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Date *</label>
                        <input type="date" {...register('expenseDate')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-500 mb-1">Category *</label>
